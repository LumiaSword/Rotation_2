...
top100_h1299 = h1299.rc %>%
  ...
  arrange(desc(total_count)) %>%
  head(100) %>%
  pull(transcript) # This will return the top 100 transcripts wih the most reads amoung all region, including UTR5, CDS and UTR3
```
...
h1299_cov_exp1_wide = h1299_cov_exp1 %>%
  group_by(transcript) %>%
  arrange(region)  %>%
...
h1299_cov_exp1_matrix_data = as.matrix(h1299_cov_exp1_wide[,-1])  # rm transcript col
rownames(h1299_cov_exp1_matrix_data) = h1299_cov_exp1_wide$transcript  

annotation_col = data.frame(
  Region = ifelse(grepl("UTR5", colnames(h1299_cov_exp1_matrix_data)), "UTR5",
                  ifelse(grepl("CDS", colnames(h1299_cov_exp1_matrix_data)), "CDS", "UTR3"))
)
rownames(annotation_col) <- colnames(h1299_cov_exp1_matrix_data)
```


```{r}
pheatmap(
  log10(h1299_cov_exp1_matrix_data + 1),
